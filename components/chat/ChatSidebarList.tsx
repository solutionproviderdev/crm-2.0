...
export function ChatSidebarList({ initialChats }: { initialChats: ChatSidebarItem[] }) {
  ...
  useEffect(() => {
    // We listen to the messages table. If ANY message is inserted 
    // where our chat_id matches, we re-fetch the sidebar data.
    const channel = supabase
      ...
      .on(
        ...
        async () => {
          // A message was received! Refetch the active chats list
          // via our Server Action to instantly update unreads and order
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {filteredChats.map((chat) => {
        const otherUser = chat.otherUsers?.[0]; // Assume direct chat
